...
# --- WEB SUNUCUSU (Render için uyanık tutar) ---
app = Flask('')
...
# --- !hesap_ekle (ÇOK SATIRLI MESAJ DESTEĞİ) ---
@bot.command()
...
async def hesap_ekle(ctx, site: str = None, *, hesaplar_metni: str = None):
    ...
    if site is None:
        await admin_dm_gonder(ctx,
            "❌ **Site belirt!**\n"
            "Örnek:\n"
            "`!hesap_ekle hesap.com.tr`\n"
            "`mail1:şifre1`\n"
            "`mail2:şifre2`"
        )
        return
    ...
    if hesaplar_metni is None:
        await admin_dm_gonder(ctx,
            "❌ **Hesapları yaz!**\n"
            "Komuttan sonra Shift+Enter ile alt satıra geçip hesapları yaz.\n"
            "Örnek:\n"
            "`!hesap_ekle hesap.com.tr`\n"
            "`mail1:şifre1`\n"
            "`mail2:şifre2`"
